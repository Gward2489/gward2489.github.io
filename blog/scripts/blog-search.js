...
document.addEventListener("click", event => {
    if (event.target.id === "searchBlogButton") {
        searchResults = []
        let searchEl = document.getElementById("blogSearchConditions")
        let blogSearch = searchEl.value
        if (blogSearch.length >= 3) {
            console.log(blogSearch)
            blogs.forEach(blog =>{
                let blogText = blog.text
                console.log(blogText)
                let searchTest = blogText.search(blogSearch)
                if (searchTest !== -1) {
                    console.log(searchTest)
                    searchResults.unshift(blog)
                } else {
                    blogArticleMarker.innerHTML = "<h1>No blogs found!</h1>"
                    pagination_footer.innerHTML = ""
                }
            })
            if (searchResults.length > 0) {
                blogArticleMarker.innerHTML = ""
                // use 'searchResults.length' to determine number of blogpost
                ...
                //style the next and previous arrow marker so it is hidden initially
                ...
                if (searchResults.length > itemsPerPage) {
                    searchResults.slice(0 , itemsPerPage).forEach(function (blog){
                        blogArticleMarker.innerHTML += `
                        <article>
                        <h1>${blog.date}</h1>
                        <h2>${blog.text}</h2>
                    ...
                    searchResults.forEach(function (blog) {
                        blogArticleMarker.innerHTML += `
                        <article>
                        <h1>${blog.date}</h1>
                        <h2>${blog.text}</h2>
                ...
                for (i = 0; i < blogLinks.length; i++) {
                    blogLinks[i].addEventListener("click", function (event) {
                        ...
                        if (currentPage === numberOfPages) {
                            whichBlogs = searchResults.slice((currentClick - 1) * itemsPerPage, (currentClick * itemsPerPage) + 1)
                        } else {
                            whichBlogs = searchResults.slice((currentClick - 1) * itemsPerPage, currentClick * itemsPerPage) 
                        }
        ...
        blogArticleMarker.innerHTML = "You must search for three or more characters!"
        pagination_footer.innerHTML =""
    }
